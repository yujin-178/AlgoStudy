...
//			System.out.println(idx % len);
				if (food[(int)(idx % len)].time != 0) {
					i++;
					if (food[(int)(idx % len)].time == (step + 1 + idx / len))
						food[(int)(idx % len)].time = 0;
//				System.out.println(food[idx % len]);
				}
			}
			while (food[(int)(idx % len)].time == 0) {
				idx++;
			}
			ans = food[(int)(idx % len)].idx;
		} else {
			ans = -1;
		}
